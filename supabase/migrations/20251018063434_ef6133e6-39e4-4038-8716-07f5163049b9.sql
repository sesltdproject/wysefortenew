-- Update process_internal_transfer function to use separate reference numbers for each transaction
...
BEGIN
  ...
  -- Create transaction record for source account (debit) with unique reference
  ...
  -- Create transaction record for destination account (credit) with unique reference
  INSERT INTO public.transactions (
    account_id,
    transaction_type,
    amount,
