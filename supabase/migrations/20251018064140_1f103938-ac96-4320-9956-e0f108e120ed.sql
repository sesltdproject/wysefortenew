-- Update process_internal_transfer to generate clearer descriptions for internal transfers
...
BEGIN
  ...
  -- Generate descriptive transaction descriptions
  v_debit_description := 'Internal Transfer - From ' || INITCAP(v_from_account_type) || ' (' || v_from_account_number || ') to ' || INITCAP(v_to_account_type) || ' (' || v_to_account_number || ')';
  v_credit_description := 'Internal Transfer - From ' || INITCAP(v_from_account_type) || ' (' || v_from_account_number || ') to ' || INITCAP(v_to_account_type) || ' (' || v_to_account_number || ')';
  ...
  -- Create transaction record for source account (debit) with descriptive text
  ...
  -- Create transaction record for destination account (credit) with descriptive text
  INSERT INTO public.transactions (
    account_id,
    transaction_type,
    amount,
