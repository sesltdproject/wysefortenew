-- Fix current state: Enable Resend as default if both providers are off
UPDATE website_settings 
SET resend_enabled = true, smtp_enabled = false
WHERE smtp_enabled = false AND resend_enabled = false;

-- Drop and recreate the function to remove email toggle dependency
DROP FUNCTION IF EXISTS update_website_settings(jsonb);

CREATE FUNCTION update_website_settings(settings jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Get existing record
  SELECT * INTO existing_record FROM website_settings LIMIT 1;
  ...
  -- Boolean values with explicit presence checks
  v_email_alerts := CASE 
    WHEN settings ? 'email_alerts_enabled' THEN (settings->>'email_alerts_enabled')::boolean 
    ELSE existing_record.email_alerts_enabled 
  END;
  
  v_auth_emails := CASE 
    WHEN settings ? 'auth_emails_enabled' THEN (settings->>'auth_emails_enabled')::boolean 
    ELSE existing_record.auth_emails_enabled 
  END;
  
  v_show_nav := CASE 
    WHEN settings ? 'show_navigation_menu' THEN (settings->>'show_navigation_menu')::boolean 
    ELSE existing_record.show_navigation_menu 
  END;
  
  v_website_vis := CASE 
    WHEN settings ? 'website_visibility' THEN (settings->>'website_visibility')::boolean 
    ELSE existing_record.website_visibility 
  END;
  
  v_show_kyc := CASE 
    WHEN settings ? 'show_kyc_page' THEN (settings->>'show_kyc_page')::boolean 
    ELSE existing_record.show_kyc_page 
  END;
  
  -- SMTP Settings
  v_smtp_enabled := CASE 
    WHEN settings ? 'smtp_enabled' THEN (settings->>'smtp_enabled')::boolean 
    ELSE existing_record.smtp_enabled 
  END;
  ...
  v_smtp_use_ssl := CASE 
    ...
    ELSE existing_record.smtp_use_ssl 
  END;
  
  v_resend_enabled := CASE 
    WHEN settings ? 'resend_enabled' THEN (settings->>'resend_enabled')::boolean 
    ELSE existing_record.resend_enabled 
  END;
  ...
  v_receipt_show_logo := CASE 
    WHEN settings ? 'receipt_show_logo' THEN (settings->>'receipt_show_logo')::boolean 
    ELSE COALESCE(existing_record.receipt_show_logo, true)
  END;
  v_receipt_show_watermark := CASE 
    WHEN settings ? 'receipt_show_watermark' THEN (settings->>'receipt_show_watermark')::boolean 
    ELSE COALESCE(existing_record.receipt_show_watermark, false)
  END;
  ...
  -- If both are being set to false, default to Resend
