-- Fix Website Settings saving: remove overloaded RPC signatures that cause PostgREST ambiguity
-- Keep a single function signature: update_website_settings(settings json)

DROP FUNCTION IF EXISTS public.update_website_settings(jsonb);
...
BEGIN
  ...
  v_receipt_header_color := COALESCE(s->>'receipt_header_color', existing_record.receipt_header_color, '#003366');
  v_receipt_accent_color := COALESCE(s->>'receipt_accent_color', existing_record.receipt_accent_color, '#22c55e');
  v_receipt_title := COALESCE(s->>'receipt_title', existing_record.receipt_title, 'Transfer Confirmation Receipt');
  v_receipt_show_logo := CASE WHEN s ? 'receipt_show_logo' THEN (s->>'receipt_show_logo')::boolean ELSE COALESCE(existing_record.receipt_show_logo, true) END;
  v_receipt_show_watermark := CASE WHEN s ? 'receipt_show_watermark' THEN (s->>'receipt_show_watermark')::boolean ELSE COALESCE(existing_record.receipt_show_watermark, false) END;
  v_receipt_watermark_text := COALESCE(s->>'receipt_watermark_text', existing_record.receipt_watermark_text, 'COPY');
  v_receipt_footer_disclaimer := COALESCE(s->>'receipt_footer_disclaimer', existing_record.receipt_footer_disclaimer, 'This is a computer-generated receipt and is valid without signature.');
  ...
  v_receipt_reference_prefix := COALESCE(s->>'receipt_reference_prefix', existing_record.receipt_reference_prefix, 'TXN');
...
-- Reduce exposure: the update should require an authenticated session (RLS still enforces admin for write).
REVOKE ALL ON FUNCTION public.update_website_settings(json) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_website_settings(json) TO authenticated;
