-- Drop the old 7-argument overload of process_intrabank_transfer
DROP FUNCTION IF EXISTS public.process_intrabank_transfer(uuid,uuid,numeric,numeric,text,text,text);

-- Drop and recreate the 5-argument version with authorization check
...
BEGIN
  ...
  -- Get recipient info for transaction records
  DECLARE
