-- Update security code functions to use fully qualified pgcrypto function names

-- Drop and recreate update_security_code function with qualified function names
DROP FUNCTION IF EXISTS public.update_security_code(uuid, text, text);
...
BEGIN
  ...
  IF v_security_record.security_code_enabled = true AND v_security_record.security_code_hash IS NOT NULL THEN
    IF p_old_code IS NULL OR extensions.crypt(p_old_code, v_security_record.security_code_hash) != v_security_record.security_code_hash THEN
      RETURN json_build_object('success', false, 'error', 'Invalid current security code');
    END IF;
  END IF;
  
  -- Hash new code
  v_new_hash := extensions.crypt(p_new_code, extensions.gen_salt('bf'));
...
-- Drop and recreate verify_security_code function with qualified function names
...
BEGIN
  ...
  FROM public.security_code_attempts 
  WHERE user_id = p_user_id 
  ...
  IF v_recent_attempts >= 5 THEN
    ...
    INSERT INTO public.security_code_attempts (user_id, attempt_type, success, ip_address, user_agent)
  ...
  IF v_security_record.security_code_hash IS NOT NULL AND 
     extensions.crypt(p_code, v_security_record.security_code_hash) = v_security_record.security_code_hash THEN
  ...
  FOR i IN 0..jsonb_array_length(v_backup_codes) - 1 LOOP
    ...
    IF v_code_hash IS NOT NULL AND extensions.crypt(p_code, v_code_hash) = v_code_hash THEN
...
-- Drop and recreate generate_backup_codes function with qualified function names
DROP FUNCTION IF EXISTS public.generate_backup_codes(uuid);
