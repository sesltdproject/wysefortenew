...
-- Add console_logo_url column
ALTER TABLE public.website_settings ADD COLUMN IF NOT EXISTS console_logo_url text;

-- Drop and recreate get_public_website_settings with new return type
DROP FUNCTION IF EXISTS public.get_public_website_settings();

CREATE OR REPLACE FUNCTION public.get_public_website_settings()
 RETURNS TABLE(id uuid, bank_name text, bank_email text, bank_phone text, bank_address text, support_email text, logo_url text, favicon_url text, primary_color text, secondary_color text, footer_logo_url text, show_navigation_menu boolean, website_visibility boolean, show_kyc_page boolean, receipt_header_color text, receipt_accent_color text, receipt_title text, receipt_watermark_text text, receipt_footer_disclaimer text, receipt_custom_message text, receipt_reference_prefix text, receipt_show_logo boolean, receipt_show_watermark boolean, console_logo_url text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $$
  SELECT 
    id, bank_name, bank_email, bank_phone, bank_address, support_email,
    logo_url, favicon_url, primary_color, secondary_color, footer_logo_url,
    show_navigation_menu, website_visibility, show_kyc_page,
    receipt_header_color, receipt_accent_color, receipt_title,
    receipt_watermark_text, receipt_footer_disclaimer, receipt_custom_message,
    receipt_reference_prefix, receipt_show_logo, receipt_show_watermark,
    console_logo_url
  FROM public.website_settings
  LIMIT 1;
$$;

-- Recreate the JSON-based update_website_settings to handle console_logo_url
...
DECLARE
  ...
  v_bank_name text; v_bank_address text; v_bank_phone text; v_support_email text;
  v_logo_url text; v_favicon_url text; v_footer_logo_url text; v_console_logo_url text;
  v_email_alerts boolean; v_auth_emails boolean; v_show_nav boolean; v_website_vis boolean; v_show_kyc boolean;
  v_smtp_enabled boolean; v_smtp_host text; v_smtp_port integer; v_smtp_username text;
  v_smtp_password text; v_smtp_from_email text; v_smtp_from_name text; v_smtp_use_ssl boolean;
  v_resend_enabled boolean;
  v_receipt_header_color text; v_receipt_accent_color text; v_receipt_title text;
  v_receipt_show_logo boolean; v_receipt_show_watermark boolean; v_receipt_watermark_text text;
  v_receipt_footer_disclaimer text; v_receipt_custom_message text; v_receipt_reference_prefix text;
  v_login_alerts_enabled boolean; v_login_alert_email text;
...
BEGIN
  ...
  v_console_logo_url := CASE WHEN s ? 'console_logo_url' THEN s->>'console_logo_url' ELSE existing_record.console_logo_url END;
  ...
  UPDATE website_settings SET
    bank_name = v_bank_name, bank_address = v_bank_address, bank_phone = v_bank_phone,
    support_email = v_support_email, logo_url = v_logo_url, favicon_url = v_favicon_url,
    footer_logo_url = v_footer_logo_url, console_logo_url = v_console_logo_url,
    email_alerts_enabled = v_email_alerts, auth_emails_enabled = v_auth_emails,
    show_navigation_menu = v_show_nav, website_visibility = v_website_vis, show_kyc_page = v_show_kyc,
    smtp_enabled = v_smtp_enabled, smtp_host = v_smtp_host, smtp_port = v_smtp_port,
    smtp_username = v_smtp_username, smtp_password = v_smtp_password,
    smtp_from_email = v_smtp_from_email, smtp_from_name = v_smtp_from_name, smtp_use_ssl = v_smtp_use_ssl,
    resend_enabled = v_resend_enabled,
    receipt_header_color = v_receipt_header_color, receipt_accent_color = v_receipt_accent_color,
    receipt_title = v_receipt_title, receipt_show_logo = v_receipt_show_logo,
    receipt_show_watermark = v_receipt_show_watermark, receipt_watermark_text = v_receipt_watermark_text,
    receipt_footer_disclaimer = v_receipt_footer_disclaimer, receipt_custom_message = v_receipt_custom_message,
    receipt_reference_prefix = v_receipt_reference_prefix,
    login_alerts_enabled = v_login_alerts_enabled, login_alert_email = v_login_alert_email,
    updated_at = now()
